...
def test(cfg):
    ...
    weights = os.listdir(f"{path}{ex}/{name}")
    ...
    for weight in weights:
        ...
        trainer = Trainer(callbacks=[early_stopping_callback, checkpoint_callback,lr_monitor],
                        logger=pl.loggers.WandbLogger(project="Master_thesis"),
        ...
        test_results= trainer.test(model,test_loaders)
        wandb.finish()
    ...
    #name = name.replace('iemocap','mer4')
    results_df.to_csv(f'/work3/s194644/results/fusion/{ex}/{name}.csv', index=False)
