...
def train(cfg):
    ...
    ex = "fusion"
    save_path = f"/work3/s194644/models/{ex}/{name}"
    multi = True
    if cparams.dataset =="iemocap":
        ...
        path = "/work3/s194644/data/mer/"
        dataset = MER4(path, batch_size=hparams.batch_size,num_class=cparams.num_classes,type1=mparams.type,no_samples=2765)
    ...
    for ii in range(len(train_loaders)):
        ...
        model = classifier(model_name=mparams.model_name,multi=multi,learning_rate=hparams.lr,h_dim=hparams.h_dim,num_classes=cparams.num_classes,dropout_prob=drparams.dropout,scheduler="test",scheduler_patience="test", scheduler_factor="test",a=dparams.a,t=dparams.t,v=dparams.v,type=mparams.type)
        # save best model 
        checkpoint_callback = ModelCheckpoint(dirpath=f"{save_path}", monitor="val_loss", mode="min")

        # add early stopping
        early_stopping_callback = EarlyStopping(monitor="val_loss", patience=25, verbose=True, mode="min")
        ...
        if multi:
            if cparams.dataset =="iemocap":
                test_mse_scores_0.append(test_results[0]['test_mse'])
                test_f1_scores_0.append(test_results[0]['test_f1'])
            else:
                test_mse_scores_0.append(test_results[0]['test_mse/dataloader_idx_0'])
                test_f1_scores_0.append(test_results[0]['test_f1/dataloader_idx_0'])
                test_mse_scores_1.append(test_results[1]['test_mse/dataloader_idx_1'])
    ...
    if multi:
        if cparams.dataset =="iemocap":
            ...
            "IEMOCAP mse": [mean_std_str(test_mse_scores_0)]})
        else:
            # Save metrics to CSV in the desired format
            results_df = pd.DataFrame({
                "experiment":name,
                "MER-MULTI f1": [mean_std_str(test_f1_scores_0)],
                "MER-MULTI mse": [mean_std_str(test_mse_scores_0)],
                "MER-NOISE f1": [mean_std_str(test_f1_scores_1)],
                "MER-NOISE mse": [mean_std_str(test_mse_scores_1)],
    ...
    results_df.to_csv(f'/work3/s194644/results/{ex}/{name}.csv', index=False)
